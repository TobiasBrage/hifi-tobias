...
module.exports = function (app) {
    ...
    app.get('/user/token/:id', function (req, res) {
        let tokenId = req.params.id.replace("id=", "");

        db.execute(`SELECT COUNT(*) as 'user' FROM userToken WHERE sessionName = '${tokenId}'`, function (err, tokenUserResult) {
            if (tokenUserResult[0].user == 1) {
                db.query(`SELECT * FROM userToken WHERE sessionName = '${tokenId}'`, function (err, data) {
                    res.json({"message": "tokenExcist", "userId": data[0].userId});
                })
            } else {
                res.json({"message": "tokenDontExcist"});
            }
        })
    });
    ...
    app.post('/cmslogin', function (req, res) {
        var num_chars = 48;
                        ...
                        db.execute(loginPassword, function (err, resultPassword) {
                            if (resultPassword[0].user == 1) {
                                db.execute(getUserId, function (err, userData) {
                                    ...
                                    db.execute(`SELECT COUNT(*) as 'user' FROM userToken WHERE userId = '${userData[0].id}'`, function (err, sessionUser) {
                                        if(sessionUser[0].user > 0) {
                                            db.query(`delete from userToken where userId = ${userData[0].id}`, function (err, data) {
                                                db.query(`INSERT INTO userToken (id, sessionName, userId) VALUES (null, '${random}', '${userData[0].id}')`, function (err, data) {
                                                    res.json({"message": "userLoggedIn", "userId": userData[0].id,"cookieSession": random});
                                                })
                                            })
                                        } else {
                                            db.query(`INSERT INTO userToken (id, sessionName, userId) VALUES (null, '${random}', '${userData[0].id}')`, function (err, data) {
